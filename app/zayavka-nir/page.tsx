...
export default function ZayavkaNIR() {
    ...
    const YesNoRadio = ({ name, label }: { name: string; label: string }) => (
        <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-purple-50 transition">
            <span className="text-gray-700 flex-1 pr-4">{label}</span>
            <div className="flex gap-4 flex-shrink-0">
                <label className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name={name} value="yes" checked={formData[name as keyof typeof formData] === 'yes'} onChange={handleInputChange} className="w-4 h-4 text-purple-600 focus:ring-purple-500" />
                    <span className="text-gray-700">Да</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name={name} value="no" checked={formData[name as keyof typeof formData] === 'no'} onChange={handleInputChange} className="w-4 h-4 text-purple-600 focus:ring-purple-500" />
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
            ...
            <main className="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
                <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
                    <div className="bg-gradient-to-r from-purple-600 via-purple-700 to-blue-700 px-6 sm:px-8 py-6 sm:py-8">
                        <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2">Заявка на проведение НИР по фармацевтической разработке</h1>
                        <p className="text-purple-100 text-sm">Сертификат на соответствие ГОСТ 33044-2014 № 19.0291.026 • Система менеджмента сертифицирована с 2016 г.</p>
                    </div>

                    <form onSubmit={handleSubmit} className="px-6 sm:px-8 py-6 sm:py-8 space-y-8">

                        <section>
                            <h2 className="text-xl font-bold text-gray-900 mb-4 pb-2 border-b-2 border-purple-600">Информация о Спонсоре исследования</h2>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="md:col-span-2">
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Название организации *</label>
                                    <input type="text" name="organizationName" value={formData.organizationName} onChange={handleInputChange} required className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition" />
                                </div>
                                <div className="md:col-span-2">
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Адрес организации *</label>
                                    <input type="text" name="organizationAddress" value={formData.organizationAddress} onChange={handleInputChange} required className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">ФИО контактного лица *</label>
                                    <input type="text" name="contactPerson" value={formData.contactPerson} onChange={handleInputChange} required className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Электронная почта *</label>
                                    <input type="email" name="contactEmail" value={formData.contactEmail} onChange={handleInputChange} required className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Телефон *</label>
                                    <input type="tel" name="contactPhone" value={formData.contactPhone} onChange={handleInputChange} required className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Дата оформления заявки</label>
                                    <input type="date" name="applicationDate" value={formData.applicationDate} onChange={handleInputChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition" />
                                </div>
                            </div>
                        </section>

                        <section>
                            <h2 className="text-xl font-bold text-gray-900 mb-4 pb-2 border-b-2 border-purple-600">1. Информация о планируемом к разработке препарате</h2>

                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Какой препарат планируется к разработке *</label>
                                    <select name="drugType" value={formData.drugType} onChange={handleInputChange} required className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <input type="text" name="dosage" value={formData.dosage} onChange={handleInputChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition" />
                                    </div>
                                    <div className="md:col-span-2">
                                        <label className="block text-sm font-medium text-gray-700 mb-1">Планируемая первичная упаковка</label>
                                        <input type="text" name="primaryPackaging" value={formData.primaryPackaging} onChange={handleInputChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition" />
                                    </div>
                                </div>

                                <div className="bg-purple-50 rounded-xl p-4 mt-6">
                                    <h3 className="text-lg font-semibold text-purple-800 mb-4">Информация об активной фармацевтической субстанции</h3>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-1">Действующее вещество(а)</label>
                                            <input type="text" name="activeSubstance" value={formData.activeSubstance} onChange={handleInputChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition bg-white" />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-1">Фармакологическая группа</label>
                                            <input type="text" name="pharmacologicalGroup" value={formData.pharmacologicalGroup} onChange={handleInputChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition bg-white" />
                                        </div>
                                        <div className="md:col-span-2">
                                            <label className="block text-sm font-medium text-gray-700 mb-1">Химическая группа, химическая структура, формула(ы)</label>
                                            <textarea name="chemicalStructure" value={formData.chemicalStructure} onChange={handleInputChange} rows={2} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition bg-white"></textarea>
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-1">Методы количественного анализа</label>
                                            <input type="text" name="quantitativeAnalysisMethods" value={formData.quantitativeAnalysisMethods} onChange={handleInputChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition bg-white" />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-1">Физико-химические свойства</label>
                                            <input type="text" name="physicochemicalProperties" value={formData.physicochemicalProperties} onChange={handleInputChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition bg-white" />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-1">Условия хранения</label>
                                            <input type="text" name="storageConditions" value={formData.storageConditions} onChange={handleInputChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition bg-white" />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-1">Информация о стабильности</label>
                                            <input type="text" name="stabilityInfo" value={formData.stabilityInfo} onChange={handleInputChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition bg-white" />
                                        </div>
                                        <div className="md:col-span-2">
                                            <label className="block text-sm font-medium text-gray-700 mb-1">Стадия отработки технологии получения субстанции</label>
                                            <select name="technologyStage" value={formData.technologyStage} onChange={handleInputChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition bg-white">
                                ...
                                <div className="bg-blue-50 rounded-xl p-4 mt-6">
                                    <h3 className="text-lg font-semibold text-blue-800 mb-4">Информация о планируемом производстве препарата</h3>
                                    <div className="space-y-4">
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-1">Производственная площадка</label>
                                            <input type="text" name="productionSite" value={formData.productionSite} onChange={handleInputChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition bg-white" />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-1">Оборудование, планируемое для производства препарата</label>
                                            <textarea name="productionEquipment" value={formData.productionEquipment} onChange={handleInputChange} rows={2} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition bg-white"></textarea>
                                        </div>
                                        <div>
                                            ...
                                            <textarea name="pilotBatchEquipment" value={formData.pilotBatchEquipment} onChange={handleInputChange} rows={2} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition bg-white"></textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <section>
                            <h2 className="text-xl font-bold text-gray-900 mb-4 pb-2 border-b-2 border-purple-600">2. Информация о требуемом объеме работ по фармацевтической разработке</h2>
                        ...
                        <section>
                            <h2 className="text-xl font-bold text-gray-900 mb-4 pb-2 border-b-2 border-purple-600">3. Для выполнения исследования будут предоставлены</h2>
                            <p className="text-sm text-gray-600 mb-4">Отметьте позиции, которые будут Вами предоставлены до начала исследования</p>
                        ...
                        <div className="pt-6 border-t border-gray-200">
                            <button type="submit" className="w-full sm:w-auto px-8 py-4 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all transform hover:scale-105">
                                Отправить заявку
                            </button>
