...
function formatContent(content: string): string {
    if (/<[a-z][\s\S]*>/i.test(content)) return cleanContent(content)
...
function saveNews(news: NewsItem[]) {
    ...
    fs.writeFileSync(NEWS_FILE, JSON.stringify(sortByDate(news), null, 2), 'utf-8')
}
...
export async function GET() {
    return NextResponse.json(sortByDate(getNews()))
}
...
export async function POST(req: NextRequest) {
    ...
    const formattedContent = formatContent(body.content)
    const newItem: NewsItem = {
        id: Date.now().toString(),
        date: dateStr,
        title: body.title,
        excerpt: body.excerpt || formattedContent.replace(/<[^>]*>/g, '').slice(0, 200) + '...',
...
export async function PUT(req: NextRequest) {
    ...
    const body = await req.json()
    if (!body.id) return NextResponse.json({ error: 'ID не указан' }, { status: 400 })

    const news = getNews()
    const index = news.findIndex(n => n.id === body.id)
    if (index === -1) return NextResponse.json({ error: 'Новость не найдена' }, { status: 404 })

    if (body.title !== undefined) news[index].title = body.title
    if (body.content !== undefined) {
        news[index].content = formatContent(body.content)
        news[index].excerpt = body.excerpt || news[index].content.replace(/<[^>]*>/g, '').slice(0, 200) + '...'
    }
